...
struct StubUsersListInteractor: UsersListInteractor {
    ...
    func refreshUsersList(users: LoadableSubject<[User]>, search: String) {
        ...
        Just<[User]>([User(name: search, userId: 0, avatar: nil),User(name: "\(search)2", userId: 1, avatar: nil)]).sinkToLoadable {
            users.wrappedValue = $0
        }.store(in: cancelBag)
    }
    ...
    func loadDetails(userDetails: LoadableSubject<User.Details>, user: User) {
        ...
        Just<User.Details>(User.Details(name: user.name, userName: user.name, avatar: user.avatar, publicRepos: 0, followers: 0)).sinkToLoadable {
            userDetails.wrappedValue = $0
        }.store(in: cancelBag)
    }
}
